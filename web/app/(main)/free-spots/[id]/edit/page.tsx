...
import { getFreeSpotById, updateFreeSpot } from '@/lib/free-spot-api';
...
export default function EditFreeSpotPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Check ownership
        if (spot.author?._id !== user?._id) {
          toast.error('Bạn không có quyền chỉnh sửa địa điểm này');
          router.push(`/free-spots/${id}`);
          return;
        }
        ...
        setCity(spot.city || '');
        setProvince(spot.province || '');
        setTerrain(spot.terrain || 'other');
        setDirections(spot.directions || '');
        setAmenities(spot.amenities || []);
        setExistingImages(spot.images || []);
        if (spot.location?.coordinates) {
          setLongitude(spot.location.coordinates[0]);
          setLatitude(spot.location.coordinates[1]);
        }
      } catch {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--background)' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ marginTop: 12, color: 'var(--muted-foreground)' }}>Đang tải...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--background)', paddingBottom: 60 }}>
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%)', padding: '28px 24px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <Link
            href={`/free-spots/${id}`}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: 13, marginBottom: 16 }}
          >
            <ArrowLeft size={14} /> Quay lại địa điểm
          </Link>
          ...
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 13, marginTop: 6, marginBottom: 0 }}>
            Cập nhật thông tin địa điểm cắm trại của bạn
          </p>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <Section title="Thông tin cơ bản">
            <FormField label="Tên địa điểm *">
              <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="VD: Bãi cắm trại bờ hồ Tuyền Lâm" style={inputStyle} maxLength={120} />
            </FormField>
            <FormField label="Mô tả *">
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Mô tả chi tiết về địa điểm..." rows={4} style={{ ...inputStyle, resize: 'vertical' }} maxLength={2000} />
              <div style={{ textAlign: 'right', fontSize: 11, color: 'var(--muted-foreground)', marginTop: 4 }}>{description.length}/2000</div>
            </FormField>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <FormField label="Địa chỉ *">
                <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Số thôn, xã, huyện..." style={inputStyle} />
              </FormField>
          ...
          <Section title="Loại địa hình">
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {TERRAIN_OPTIONS.map((opt) => (
                <button key={opt.value} type="button" onClick={() => setTerrain(opt.value)} style={{
          ...
          <Section title="Vị trí trên bản đồ *">
            <div style={{ padding: '10px 14px', borderRadius: 10, marginBottom: 14, background: '#eff6ff', border: '1px solid #bfdbfe', display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 13 }}>
              <Info size={15} color="#2563eb" style={{ marginTop: 1, flexShrink: 0 }} />
              <span style={{ color: '#1e40af', lineHeight: 1.5 }}>Click vào bản đồ để thay đổi vị trí, hoặc kéo marker.</span>
            </div>
            <FreeSpotMap
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <div style={{ flex: 1, minWidth: 140 }}>
                ...
                <input type="number" value={latitude} onChange={(e) => setLatitude(e.target.value ? parseFloat(e.target.value) : '')} placeholder="VD: 11.9404" step="0.000001" style={inputStyle} />
              </div>
              <div style={{ flex: 1, minWidth: 140 }}>
                <label style={labelStyle}>Kinh độ (Longitude)</label>
                <input type="number" value={longitude} onChange={(e) => setLongitude(e.target.value ? parseFloat(e.target.value) : '')} placeholder="VD: 108.4358" step="0.000001" style={inputStyle} />
              </div>
              <button type="button" onClick={handleGetLocation} disabled={locating} style={{
                ...
                color: '#2563eb', fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap',
              }}>
          ...
          <Section title="Tiện ích có sẵn">
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {AMENITY_OPTIONS.map((opt) => (
                <button key={opt.value} type="button" onClick={() => toggleAmenity(opt.value)} style={{
          ...
          <Section title="Hướng dẫn di chuyển">
            <textarea value={directions} onChange={(e) => setDirections(e.target.value)} placeholder="Mô tả cách đến địa điểm..." rows={5} style={{ ...inputStyle, resize: 'vertical' }} maxLength={3000} />
          </Section>

          {/* Existing Images */}
          {existingImages.length > 0 && (
            <Section title="📷 Ảnh hiện tại">
              <p style={{ fontSize: 12, color: 'var(--muted-foreground)', marginBottom: 12 }}>
                Nếu bạn chọn ảnh mới bên dưới, ảnh cũ sẽ bị thay thế.
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10 }}>
                {existingImages.map((url, i) => (
                  <div key={i} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', aspectRatio: '1', background: '#e5e7eb' }}>
          ...
          {/* New Images */}
          <Section title="📷 Thêm ảnh mới (tùy chọn, tối đa 8 ảnh)">
            <div
              onClick={() => fileRef.current?.click()}
              style={{
                border: '2px dashed var(--border)', borderRadius: 12,
                padding: 32, textAlign: 'center', cursor: 'pointer',
                background: 'var(--muted)', marginBottom: newPreviews.length ? 16 : 0,
              }}
            ...
            >
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--foreground)', marginBottom: 4 }}>
                Click để chọn ảnh mới hoặc kéo thả
              </div>
              <div style={{ fontSize: 12, color: 'var(--muted-foreground)' }}>PNG, JPG, WebP</div>
